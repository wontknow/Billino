...
from models import Profile, Customer, Invoice, SummaryInvoice, SummaryInvoiceLink
...
def sample_data(session):
    ...
    # Create profiles
    profile = Profile(name="Test Profile", address="Test Address", city="Test City", 
                     include_tax=True, default_tax_rate=0.19)
    profile_no_tax = Profile(name="No Tax Profile", address="Test Address", city="Test City", 
                            include_tax=False, default_tax_rate=0.0)
    
    # Create customer
    customer = Customer(name="Test Customer", address="Customer Address", city="Customer City")
    ...
    inv3 = Invoice(number="25 | 0003", date="2025-10-03", profile_id=profile_no_tax.id,
                   customer_id=customer.id, include_tax=False, is_gross_amount=True, 
                   tax_rate=0.0, total_amount=50.0)
...
def test_create_summary_invoice_wrong_profile_invoices(client, sample_data):
    ...
    payload = {
        "profile_id": data["profile"].id,
        "invoice_ids": [data["invoices"][2].id]  # This invoice belongs to profile_no_tax
    }
...
def test_create_summary_invoice_invalid_data_types(client):
    ...
    payload = {
        "profile_id": "not_an_integer",
        "invoice_ids": ["not", "integers"]
    }
...
def test_get_summary_invoice_with_linked_invoices(client, sample_data):
    ...
    result = response.json()
    assert "invoice_ids" in result
    assert set(result["invoice_ids"]) == {data["invoices"][0].id, data["invoices"][1].id}
...
def test_delete_summary_invoice_cascade_links(client, sample_data, session):
    ...
    links_before = session.exec(
        select(SummaryInvoiceLink).where(SummaryInvoiceLink.summary_invoice_id == summary_id)
    ).all()
...
def test_delete_summary_invoice_preserves_original_invoices(client, sample_data, session):
...
def test_concurrent_operations(client, sample_data):
    ...
    payloads = [
        ...
        {"profile_id": data["profile_no_tax"].id, "invoice_ids": [data["invoices"][2].id]}
    ]
