...
from models import SummaryInvoiceLink, Profile, Invoice, Customer, SummaryInvoiceCreate
...
def test_create_summary_invoice_calculations(session: Session):
    ...
    profile = Profile(name="Testprofil", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    customer = Customer(name="Max Mustermann")
    session.add(profile)
    session.add(customer)
    session.commit()

    inv1 = Invoice(number="25 | 0001", date="2025-10-01", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=True, tax_rate=0.19, total_amount=119.0)
    inv2 = Invoice(number="25 | 0002", date="2025-10-02", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=False, tax_rate=0.19, total_amount=100.0)
    session.add_all([inv1, inv2])
    session.commit()
    summary = SummaryInvoiceCreate(profile_id=profile.id, invoice_ids=[inv1.id, inv2.id])
    # Act
    ...
    links = session.exec(select(SummaryInvoiceLink).filter_by(summary_invoice_id=summary.id)).all()
    assert len(links) == 2
...
def test_create_summary_invoice_single_invoice(session: Session):
    ...
    invoice = Invoice(number="25 | 0001", date="2025-10-01", profile_id=profile.id,
                     customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                     tax_rate=0.19, total_amount=119.0)
    session.add(invoice)
    session.commit()

    summary = create_summary_invoice(session, summary=SummaryInvoiceCreate(profile_id=profile.id, invoice_ids=[invoice.id]))
...
def test_create_summary_invoice_no_tax_kleinunternehmer(session: Session):
    ...
    profile = Profile(name="Kleinunternehmer", address="X", city="Y", include_tax=False, default_tax_rate=0.0)
    customer = Customer(name="Max Mustermann")
    session.add_all([profile, customer])
    session.commit()

    inv1 = Invoice(number="25 | 0001", date="2025-10-01", profile_id=profile.id,
                   customer_id=customer.id, include_tax=False, is_gross_amount=True, 
                   tax_rate=0.0, total_amount=100.0)
    inv2 = Invoice(number="25 | 0002", date="2025-10-02", profile_id=profile.id,
                   customer_id=customer.id, include_tax=False, is_gross_amount=True, 
                   tax_rate=0.0, total_amount=200.0)
    session.add_all([inv1, inv2])
    session.commit()

    summary = create_summary_invoice(session, summary=SummaryInvoiceCreate(profile_id=profile.id, invoice_ids=[inv1.id, inv2.id]))
...
def test_create_summary_invoice_mixed_tax_rates(session: Session):
    ...
    profile = Profile(name="Mixed Tax", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    customer = Customer(name="Max Mustermann")
    session.add_all([profile, customer])
    session.commit()
    ...
    # 19% Steuer
    inv1 = Invoice(number="25 | 0001", date="2025-10-01", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=False, 
                   tax_rate=0.19, total_amount=100.0)
    # 7% Steuer (ermäßigt)
    inv2 = Invoice(number="25 | 0002", date="2025-10-02", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=False, 
                   tax_rate=0.07, total_amount=100.0)
    session.add_all([inv1, inv2])
    session.commit()
...
def test_create_summary_invoice_mixed_gross_net_amounts(session: Session):
    ...
    profile = Profile(name="Mixed Amounts", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    customer = Customer(name="Max Mustermann")
    session.add_all([profile, customer])
    session.commit()
...
def test_create_summary_invoice_large_number_range(session: Session):
    ...
    profile = Profile(name="Large Range", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    customer = Customer(name="Max Mustermann")
    session.add_all([profile, customer])
    session.commit()

    inv1 = Invoice(number="25 | 0005", date="2025-10-01", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                   tax_rate=0.19, total_amount=119.0)
    inv2 = Invoice(number="25 | 0050", date="2025-10-02", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                   tax_rate=0.19, total_amount=119.0)
    session.add_all([inv1, inv2])
    session.commit()
...
def test_create_summary_invoice_unordered_invoice_numbers(session: Session):
    ...
    profile = Profile(name="Unordered", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    customer = Customer(name="Max Mustermann")
    session.add_all([profile, customer])
    session.commit()

    # Rechnungen in ungeordneter Reihenfolge
    inv1 = Invoice(number="25 | 0030", date="2025-10-01", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                   tax_rate=0.19, total_amount=119.0)
    inv2 = Invoice(number="25 | 0010", date="2025-10-02", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                   tax_rate=0.19, total_amount=119.0)
    inv3 = Invoice(number="25 | 0020", date="2025-10-03", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                   tax_rate=0.19, total_amount=119.0)
    session.add_all([inv1, inv2, inv3])
    session.commit()

    summary = create_summary_invoice(session, summary=SummaryInvoiceCreate(profile_id=profile.id, invoice_ids=[inv1.id, inv2.id, inv3.id]))
...
def test_create_summary_invoice_invalid_profile_id(session: Session):
    ...
    with pytest.raises(ValueError, match="Profile not found"):
        create_summary_invoice(session, summary=SummaryInvoiceCreate(profile_id=9999, invoice_ids=[1, 2]))


def test_create_summary_invoice_empty_invoice_list(session: Session):
    """Test error when invoice list is empty."""
    profile = Profile(name="Empty List", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    session.add(profile)
    session.commit()

    with pytest.raises(ValueError, match="No valid invoices found"):
        create_summary_invoice(session, summary=SummaryInvoiceCreate(profile_id=profile.id, invoice_ids=[]))


def test_create_summary_invoice_nonexistent_invoice_ids(session: Session):
    """Test error when invoice IDs don't exist."""
    profile = Profile(name="Nonexistent", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    session.add(profile)
    session.commit()

    with pytest.raises(ValueError, match="No valid invoices found"):
        create_summary_invoice(session, summary=SummaryInvoiceCreate(profile_id=profile.id, invoice_ids=[9999, 8888]))
...
def test_create_summary_invoice_wrong_profile_invoices(session: Session):
    ...
    profile1 = Profile(name="Profile 1", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    profile2 = Profile(name="Profile 2", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    customer = Customer(name="Max Mustermann")
    ...
    invoice = Invoice(number="25 | 0001", date="2025-10-01", profile_id=profile2.id,
                     customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                     tax_rate=0.19, total_amount=119.0)
    session.add(invoice)
    session.commit()
    ...
    with pytest.raises(ValueError, match="No valid invoices found"):
        create_summary_invoice(session, summary=SummaryInvoiceCreate(profile_id=profile1.id, invoice_ids=[invoice.id]))
...
def test_create_summary_invoice_partial_invalid_invoices(session: Session):
    ...
    valid_invoice = Invoice(number="25 | 0001", date="2025-10-01", profile_id=profile1.id,
                           customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                           tax_rate=0.19, total_amount=119.0)
    # Eine ungültige Invoice für profile2
    invalid_invoice = Invoice(number="25 | 0002", date="2025-10-02", profile_id=profile2.id,
                             customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                             tax_rate=0.19, total_amount=119.0)
    ...
    summary = create_summary_invoice(session, summary=SummaryInvoiceCreate(profile_id=profile1.id, invoice_ids=[valid_invoice.id, invalid_invoice.id]))
...
def test_create_summary_invoice_zero_amounts(session: Session):
    ...
    profile = Profile(name="Zero Amounts", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    customer = Customer(name="Max Mustermann")
    session.add_all([profile, customer])
    session.commit()
    ...
    invoice = Invoice(number="25 | 0001", date="2025-10-01", profile_id=profile.id,
                     customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                     tax_rate=0.19, total_amount=0.0)
    session.add(invoice)
    session.commit()
...
def test_create_summary_invoice_negative_amounts(session: Session):
    ...
    profile = Profile(name="Negative Amounts", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    customer = Customer(name="Max Mustermann")
    session.add_all([profile, customer])
    session.commit()
    ...
    inv2 = Invoice(number="25 | 0002", date="2025-10-02", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                   tax_rate=0.19, total_amount=-59.5)
    session.add_all([inv1, inv2])
    session.commit()
...
def test_create_summary_invoice_very_high_tax_rate(session: Session):
    ...
    profile = Profile(name="High Tax", address="X", city="Y", include_tax=True, default_tax_rate=0.5)
    customer = Customer(name="Max Mustermann")
    session.add_all([profile, customer])
    session.commit()
    ...
    invoice = Invoice(number="25 | 0001", date="2025-10-01", profile_id=profile.id,
                     customer_id=customer.id, include_tax=True, is_gross_amount=False, 
                     tax_rate=0.5, total_amount=100.0)  # 50% Steuer
    session.add(invoice)
    session.commit()
...
def test_create_summary_invoice_rounding_precision(session: Session):
    ...
    profile = Profile(name="Precision", address="X", city="Y", include_tax=True, default_tax_rate=0.19)
    customer = Customer(name="Max Mustermann")
    session.add_all([profile, customer])
    session.commit()
    ...
    inv1 = Invoice(number="25 | 0001", date="2025-10-01", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                   tax_rate=0.19, total_amount=33.33)
    inv2 = Invoice(number="25 | 0002", date="2025-10-02", profile_id=profile.id,
                   customer_id=customer.id, include_tax=True, is_gross_amount=True, 
                   tax_rate=0.19, total_amount=66.67)
    session.add_all([inv1, inv2])
    session.commit()
