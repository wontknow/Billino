...
class TestInvoiceCreateValidation:
    ...
    def test_is_gross_amount_without_include_tax_validation(self):
        ...
        with pytest.raises(
            ValueError, match="is_gross_amount can only be True if include_tax is True"
        ):
            InvoiceCreate(**data)
    ...
    def test_is_gross_amount_without_tax_rate_validation(self):
        ...
        with pytest.raises(
            ValueError, match="tax_rate must be provided if is_gross_amount is True"
        ):
            InvoiceCreate(**data)
    ...
    def test_include_tax_false_with_nonzero_tax_rate_validation(self):
        ...
        with pytest.raises(
            ValueError, match="tax_rate must be 0 if include_tax is False"
        ):
            InvoiceCreate(**data)
    ...
    def test_include_tax_true_without_tax_rate_validation(self):
        ...
        with pytest.raises(
            ValueError, match="tax_rate must be provided if include_tax is True"
        ):
            InvoiceCreate(**data)
...
class TestInvoiceCreateWithNumberValidation:
    ...
    def test_valid_invoice_number_formats(self):
        ...
        valid_numbers = ["25 | 001", "24 | 123", "23 | 9999", "00 | 000", "99 | 100000"]
...
class TestInvoiceItemCreateValidation:
    ...
    def test_valid_invoice_item_creation(self):
        ...
        item = InvoiceItemCreate(
            quantity=2, description="Test Item", price=50.0, tax_rate=0.19
        )
    ...
    def test_invoice_item_optional_tax_rate(self):
        ...
        item = InvoiceItemCreate(quantity=1, description="Test Item", price=100.0)
    ...
    def test_invoice_item_required_fields(self):
        ...
        with pytest.raises(ValidationError):
            InvoiceItemCreate(quantity=1, description="Test")  # Missing price

    def test_invoice_item_field_types(self):
        """Test field type validation"""
        # quantity should be int
        with pytest.raises(ValidationError):
            InvoiceItemCreate(quantity="not_an_int", description="Test", price=100.0)

        # price should be float/numeric
        with pytest.raises(ValidationError):
            InvoiceItemCreate(quantity=1, description="Test", price="not_a_number")

        # description should be string
        with pytest.raises(ValidationError):
            InvoiceItemCreate(
                quantity=1, description=123, price=100.0  # Should be string
            )
