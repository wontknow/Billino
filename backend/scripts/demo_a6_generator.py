...
def create_test_invoice_data(
    invoice_number: str, customer_name: str, amount: float = 119.0
) -> PDFInvoiceData:
    ...
    return PDFInvoiceData(
        ...
        tax_rate=0.19,
        items=[
            {"description": "Beratungsleistung", "quantity": 1, "price": round(net, 2)}
        ],
...
def main():
    ...
    try:
        ...
        print(
            f"✅ SUCCESS! Generated {os.path.basename(output_file)} ({len(pdf_bytes):,} bytes)"
        )
        print()
        print("📋 Demo Features:")
