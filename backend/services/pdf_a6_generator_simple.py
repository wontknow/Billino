...
class PDFA6GeneratorSimple:
    ...
    def __init__(self):
        ...
        self.positions = [
            ...
            (self.page_margin_x + self.a6_width, self.page_margin_y + self.a6_height),  # Top-right
        ]
    ...
    def _draw_single_invoice(self, canvas_obj, x, y, data: PDFInvoiceData):
        ...
        # Title
        story.append(Paragraph(f"Rechnung {data.invoice_number}", self.styles["A6Title"]))
        
        # Separator
        story.append(HRFlowable(width="100%", thickness=0.25, color=self.colors["accent"], spaceAfter=2*mm))
        ...
        # Date
        story.append(Paragraph(f"Datum: {data.date.strftime('%d.%m.%Y')}", self.styles["A6Small"]))
        story.append(Spacer(1, 2*mm))
        ...
        if data.items:
            ...
            for item in data.items[:3]:  # Limit to 3 items for space
                ...
                story.append(Paragraph(f"{item['quantity']}x {desc}: {total:.2f} €", self.styles["A6Small"]))
            story.append(Spacer(1, 2*mm))
        
        # Total
        story.append(Paragraph("Summe:", self.styles["A6Section"]))
        if data.total_tax > 0:
            story.append(Paragraph(f"Netto: {data.total_net:.2f} €", self.styles["A6Small"]))
            story.append(Paragraph(f"MwSt: {data.total_tax:.2f} €", self.styles["A6Small"]))
        story.append(Paragraph(f"Gesamt: {data.total_gross:.2f} €", self.styles["A6Text"]))
        ...
        canvas_obj.drawString(x + 5*mm, y + self.a6_height - 10*mm, f"Rechnung {data.invoice_number}")
        
        canvas_obj.setFont("Helvetica", 7)
        canvas_obj.drawString(x + 5*mm, y + self.a6_height - 20*mm, f"An: {data.customer_name}")
        canvas_obj.drawString(x + 5*mm, y + self.a6_height - 30*mm, f"Datum: {data.date.strftime('%d.%m.%Y')}")
        canvas_obj.drawString(x + 5*mm, y + self.a6_height - 40*mm, f"Gesamt: {data.total_gross:.2f} €")
