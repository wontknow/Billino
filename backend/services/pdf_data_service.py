...
class PDFDataService:
    ...
    def get_invoice_pdf_data_from_model(
        self, invoice: InvoiceRead, profile: Profile, customer: Customer
    ) -> PDFInvoiceData:
        """
        ...
        total_net, total_tax, total_gross = self._calculate_amounts(
            ...
            (
                invoice.include_tax
                if invoice.include_tax is not None
                else profile.include_tax
            ),
        )
    ...
    def get_summary_invoice_pdf_data_from_model(
        ...
        if isinstance(summary_invoice.date, str):
            ...
            if "T" in summary_invoice.date:
                summary_date = datetime.fromisoformat(
                    summary_invoice.date.replace("Z", "+00:00")
                ).date()
            else:
                summary_date = datetime.strptime(
                    summary_invoice.date, "%Y-%m-%d"
                ).date()
        else:
    ...
    def _format_address(
        self, address: Optional[str], city: Optional[str], name: Optional[str] = None
    ) -> str:
        """
