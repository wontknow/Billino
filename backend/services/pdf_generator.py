...
class PDFGenerator:
    ...
    def generate_invoice_pdf(self, data: PDFInvoiceData) -> bytes:
        ...
        story.append(Paragraph(f"Rechnung {data.invoice_number}", self.styles['DocumentTitle']))
        
        # Subtle separator line
        story.append(HRFlowable(width="100%", thickness=0.5, lineCap='round', 
                               color=self.colors['accent'], spaceAfter=8*mm))
        
        # Sender and customer information with professional layout
        address_data = [
            [
                Paragraph(f"<b>Rechnungssteller</b><br/><br/>{data.sender_name}<br/>{data.sender_address}", 
                         self.styles['Address']),
                Paragraph(f"<b>Rechnungsempfänger</b><br/><br/>{data.customer_address}", 
                         self.styles['Address'])
            ]
        ]
        
        address_table = Table(address_data, colWidths=[9*cm, 9*cm], rowHeights=[35*mm])
        ...
        meta_data = [
            ["Rechnungsdatum:", data.date.strftime('%d.%m.%Y')]
        ]
        ...
        if data.sender_bank_data:
            ...
            story.append(Paragraph("<b>Zahlungsinformationen</b>", self.styles['SectionHeader']))
    ...
    def generate_summary_invoice_pdf(self, data: PDFSummaryInvoiceData) -> bytes:
        ...
        # Document header with elegant styling
        story.append(Paragraph(f"Sammelrechnung {data.range_text}", self.styles['DocumentTitle']))
        ...
        if data.invoice_numbers:
            story.append(Paragraph("<b>Enthaltene Rechnungen</b>", self.styles['SectionHeader']))
